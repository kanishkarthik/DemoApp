...
import { DefaultLanguage, i18nAssetsPath, SupportedLanguage, SupportedLanguages } from '../../constants/global.constants';
...
export class TranslationService {
    ...
    private mergedRemoteLanguages: Record<string, Set<SupportedLanguage>> = {};
    ...
    async initialize(): Promise<void> {
        ...
        const browserLanguage = (navigator.language) as SupportedLanguage | null;
    ...
    private async loadLanguageData(language: SupportedLanguage): Promise<void> {
        if (this.translationData[language]) {
            this.log(`Translation already available: ${language}`);
            return;
        }
        console.log(this.config, "config");
        const url = `${this.config.appConfig.baseUrl}${i18nAssetsPath}${language}.json`;
        try {
            ...
            if (language !== 'en-US' && !this.translationData['en-US']) {
    ...
    async mergeRemoteTranslations(remoteBaseUrl: string): Promise<void> {
        ...
        this.mergedRemoteLanguages[remoteBaseUrl] ??= new Set<SupportedLanguage>();
        const supported = [...SupportedLanguages];
    ...
    private async mergeLanguageFromRemote(
        ...
        this.mergedRemoteLanguages[remoteUrl] ??= new Set<SupportedLanguage>();
        if (this.mergedRemoteLanguages[remoteUrl].has(language)) {
            this.log(`Remote translations already loaded: ${remoteUrl}${language}.json`);
            return;
        }
        ...
        try {
            ...
            this.mergedRemoteLanguages[remoteUrl].add(language);
